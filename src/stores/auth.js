  ...
  actions: {
    ...
    async fetchUser() {
      ...
      try {
        ...
        console.warn(
          'Fetch user (via service) call succeeded but API indicated failure or missing data:',
    ...
    async checkAuthStatus() {
      if (this.token && !this.user) {
        // Token exists but no user data (e.g., on app load)
    ...
    async loginWithGoogle(code) {
      ...
      try {
        ...
        const apiErrorMessage =
          response.data?.message || 'Google login failed: Invalid response from server.'
        throw new Error(apiErrorMessage)
      } catch (error) {
    ...
    async loginWithGitHub(code) {
      ...
      try {
        ...
        const apiErrorMessage =
          response.data?.message || 'GitHub login failed: Invalid response from server.'
        throw new Error(apiErrorMessage)
      } catch (error) {
